...
export function WeatherWidget({ parkCode }: Props) {
  const { data: weather, isLoading: loading } = useQuery({
	...
	if (loading) {
		return (
			<div
				className="bg-white dark:bg-stone-800 rounded-xl p-4 border border-stone-200 dark:border-stone-700 shadow-sm animate-pulse motion-reduce:animate-none"
				role="status"
	...
	if (!weather) {
		return (
			<p className="text-sm text-stone-500 dark:text-stone-400">Weather data unavailable</p>
		);
	}
  ...
  return (
    <div className="bg-white dark:bg-stone-800 rounded-xl p-4 border border-stone-200 dark:border-stone-700 shadow-sm">
      <div className="flex items-center gap-2 mb-2">
        <CloudSun className="h-5 w-5 text-park-forest" aria-hidden="true" />
