...
import { useParkEnrichment } from '@/hooks/useParks';
import { MapPin, Activity, Clock, Ticket, CloudSun, BadgeCheck, X, Accessibility, Eye, Ear, Languages, Tag, Navigation } from 'lucide-react';
...
export default function ParkModal({ park, onClose }: Props) {
  ...
  const { data: enrichedPark } = useParkEnrichment(park.parkCode);
  const resolvedPark = enrichedPark ?? park;

  const image = resolvedPark.images[0];
  const topActivities = resolvedPark.activities.slice(0, 10);
  const hasFees = resolvedPark.entranceFees.length > 0;
  const hasHours = resolvedPark.operatingHours.length > 0;
  const stateList = resolvedPark.states.split(',').join(' · ');
      ...
      <div ref={modalRef} className="relative w-full sm:max-w-2xl bg-park-cream dark:bg-stone-800 rounded-t-3xl sm:rounded-2xl shadow-2xl max-h-[92vh] overflow-y-auto z-10">
        ...
        <div className="relative h-64 sm:h-72 bg-gradient-to-br from-park-forest to-park-sage rounded-t-3xl sm:rounded-t-2xl overflow-hidden flex-shrink-0">
          {image?.url && (
            <Image
              src={image.url}
              alt={image.altText || resolvedPark.fullName}
              fill
          ...
          <a
            href={`/parks/${resolvedPark.parkCode}`}
          ...
          <div className="absolute bottom-4 left-5 right-16 pointer-events-none">
            ...
            <span className="inline-flex mb-2">
              <AlertBadge alertSummary={{
                alert_count: resolvedPark.alert_count,
                has_closure: resolvedPark.has_closure,
                has_danger: resolvedPark.has_danger,
                alert_level: resolvedPark.alert_level,
              }} />
            </span>
            <h2 id="park-modal-heading" className="text-white text-2xl font-bold leading-tight drop-shadow-md">
              {resolvedPark.fullName}
            </h2>
            {stateList && (
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="flex flex-wrap gap-3">
            <WishlistButton parkCode={resolvedPark.parkCode} />
            <VisitedButton parkCode={resolvedPark.parkCode} />
          </div>
          <AddToTripButton parkCode={resolvedPark.parkCode} />

          {(resolvedPark.has_wheelchair_access || resolvedPark.has_braille || resolvedPark.has_asl || resolvedPark.has_audio_description) && (
            <section aria-label="Accessibility features">
              <div className="flex flex-wrap gap-3">
                {resolvedPark.has_wheelchair_access && (
                  <span className="inline-flex items-center gap-1.5 text-xs bg-park-sage/15 dark:bg-park-sage/25 text-park-bark dark:text-park-cream px-3 py-1.5 rounded-full border border-park-sage/30">
                    <Accessibility className="h-3.5 w-3.5" aria-hidden="true" />
                    Wheelchair Access
                  </span>
                )}
                {resolvedPark.has_braille && (
                  <span className="inline-flex items-center gap-1.5 text-xs bg-park-sage/15 dark:bg-park-sage/25 text-park-bark dark:text-park-cream px-3 py-1.5 rounded-full border border-park-sage/30">
                    <Eye className="h-3.5 w-3.5" aria-hidden="true" />
                    Braille
                  </span>
                )}
                {resolvedPark.has_asl && (
                  <span className="inline-flex items-center gap-1.5 text-xs bg-park-sage/15 dark:bg-park-sage/25 text-park-bark dark:text-park-cream px-3 py-1.5 rounded-full border border-park-sage/30">
                    <Languages className="h-3.5 w-3.5" aria-hidden="true" />
                    ASL
                  </span>
                )}
                {resolvedPark.has_audio_description && (
                  <span className="inline-flex items-center gap-1.5 text-xs bg-park-sage/15 dark:bg-park-sage/25 text-park-bark dark:text-park-cream px-3 py-1.5 rounded-full border border-park-sage/30">
          ...
          <p className="text-stone-700 dark:text-stone-300 leading-relaxed">{resolvedPark.description}</p>
          ...
          {resolvedPark.topics.length > 0 && (
            <section>
              <h3 className="font-semibold text-park-bark dark:text-park-cream text-sm uppercase tracking-wider mb-3 flex items-center gap-2">
                <Tag className="h-4 w-4" aria-hidden="true" /> Topics
              </h3>
              <div className="flex flex-wrap gap-2">
                {resolvedPark.topics.slice(0, 10).map((topic) => (
                  <span
                    key={topic.id}
                    className="bg-stone-200 dark:bg-stone-700 text-stone-700 dark:text-stone-300 text-xs px-3 py-1.5 rounded-full"
                  >
                    {topic.name}
                  </span>
                ))}
          ...
          {hasHours && (
            <section>
              ...
              {resolvedPark.operatingHours.map((hours, index) => (
          ...
          {hasFees ? (
            <section>
              ...
              <div className="space-y-2">
                {resolvedPark.entranceFees.map((fee, index) => (
          ...
          {resolvedPark.directionsUrl && (
            <section>
              <h3 className="font-semibold text-park-bark dark:text-park-cream text-sm uppercase tracking-wider mb-2 flex items-center gap-2">
                <Navigation className="h-4 w-4" aria-hidden="true" /> Getting There
              </h3>
              {resolvedPark.directionsInfo && (
                <p className="text-sm text-stone-700 dark:text-stone-300 leading-relaxed mb-2">{resolvedPark.directionsInfo}</p>
              )}
              <a
                href={resolvedPark.directionsUrl}
                target="_blank"
                rel="noopener noreferrer"
                className="text-sm text-park-forest hover:underline"
              >
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <div className="flex-1">
              ...
              <WeatherWidget parkCode={resolvedPark.parkCode} />
            </div>
            <DistanceBadge
              parkCode={resolvedPark.parkCode}
              latitude={resolvedPark.latitude}
              longitude={resolvedPark.longitude}
            />
          </div>

          <a
            href={resolvedPark.url}
            target="_blank"
            rel="noopener noreferrer"
